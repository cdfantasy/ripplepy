...
class MGrid():
    ...
    def apply_extcur(self, extcur=None):
        '''
        Apply external coil currents to the mgrid fields.
        
        This method multiplies each coil group field by its corresponding current value,
        then sums them to create the total field.
        
        Args:
            extcur: array of coil currents. If None, uses unit currents for all coils.
                    If mgrid mode is 'S' (scaled), values are used directly.
                    If mgrid mode is 'R' (raw), values are normalized by raw coil current.
        
        Returns:
            self (for method chaining)
        '''
        if extcur is None:
            extcur = np.ones(self.n_ext_cur)
            print(f'No extcur provided, set extcur = {extcur}.')
        
        if len(extcur) != self.n_ext_cur:
            raise ValueError('Length of extcur does not match number of external coils in MGrid.')
        
        if self.mode == 'S':
            extcur = np.array(extcur)
            print(f'MGrid file is Scale mode, applied extcur = {extcur}.')
        else:
            extcur = np.array(extcur) / self.raw_coil_current
            print(f'MGrid file is Raw mode, applied extcur normalized by raw coil current = {extcur}.')
        
        # Reshape extcur for broadcasting: (n_ext_cur,) -> (n_ext_cur, 1, 1, 1)
        extcur_reshaped = extcur[:, np.newaxis, np.newaxis, np.newaxis]
        
        # Apply extcur to each coil group
        self.bp_arr = extcur_reshaped * self.bp_arr
        self.br_arr = extcur_reshaped * self.br_arr
        self.bz_arr = extcur_reshaped * self.bz_arr
        
        # Sum over coil groups to get total field
        self.bp = np.sum(self.bp_arr, axis=0)
        self.br = np.sum(self.br_arr, axis=0)
        self.bz = np.sum(self.bz_arr, axis=0)
        
        # Create combined field vector
    ...
    def expand_to_full_torus(self, nfp: int = None, full_torus: bool = True):
        '''
        Expand the mgrid data from a single field period to the full torus (2π coverage).
        
        This method performs the following operations:
        1. Tiles the magnetic field arrays (br_arr, bp_arr, bz_arr) nfp times along the phi direction
        2. Adds a closing point at phi=2π (duplicating the first phi point)
        3. Sums the field arrays to create the total field (br, bp, bz)
        4. Creates the combined field vector (bvec)
        5. Updates nphi accordingly
        
        Args:
            nfp: number of field periods (defaults to self.nfp if not provided)
            full_torus: whether to expand to full torus. If False, only sums the fields.
        ...
        # Expand to full torus if requested
        if full_torus:
            self.bp_arr = np.tile(self.bp_arr, (nfp, 1, 1))
            self.br_arr = np.tile(self.br_arr, (nfp, 1, 1))
            self.bz_arr = np.tile(self.bz_arr, (nfp, 1, 1))
            self.nphi = self.nphi * nfp
        
        # Add periodic closing point at phi=2π (duplicate first phi point)
        self.bp_arr = np.concatenate([self.bp_arr, self.bp_arr[:, 0:1]], axis=1)
        self.br_arr = np.concatenate([self.br_arr, self.br_arr[:, 0:1]], axis=1)
        self.bz_arr = np.concatenate([self.bz_arr, self.bz_arr[:, 0:1]], axis=1)
        ...
        # Update nphi to account for the closing point
        self.nphi += 1
